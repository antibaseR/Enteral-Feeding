...
logidata = data %>%  
  mutate(renal_recov = ifelse(renal_recov == 1, "Yes", "No")) %>%
  select(renal_recov, total_LA_volume_sqrt_z, 
...
test_sum_list = roc(testLabel, pred_prob$Yes) #
...
# Save the result
write.csv(glm_fit_df, "./Xinlei/Result/Secondary_Analysis_AKI_Recovery_glm_fit_result.csv")
