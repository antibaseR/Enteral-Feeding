...
data$survival_time = ifelse(data$survived_days>28, 28, data$survived_days)
...
# Logistic regression
logidata = data %>%  
  mutate(dead_28 = ifelse(dead_28 == 1, "Yes", "No")) %>%
  select(dead_28, total_LA_volume_sqrt_z, 
...
set.seed(960725)
trainIndex <- createDataPartition(completeData$dead_28, 
                                  p = .8, 
                                  list = FALSE, 
                                  times = 1)
...
trainLabel = train$dead_28
trainData = train[, -(which("dead_28"==colnames(train)))]
testLabel = test$dead_28
testData = test[, -(which("dead_28"==colnames(test)))]
...
test_sum_list = roc(testLabel, pred_prob$Yes) # 0.81
...
# Save the result
write.csv(glm_fit_df, "./Xinlei/Result/Primay_Analysis_28_Day_Mortality_Among_Sepsis_glm_fit_result.csv")



# Cox ph model
coxdata = data %>%
  select(survival_time, dead_28, total_LA_volume_sqrt_z, 
         age_at_admission, gender, race, charlson_sqrt_z, apache3_sqrt_z, 
         diabetes, SEPTIC_SHOCK, vaso_inotr,
         total_Kcal_sqrt_z)
coxdata = coxdata[complete.cases(coxdata), ]

#set.seed(960725)
#obj = rfsrc(Surv(survival_time, dead_28) ~ ., coxdata,
#ntree = 1, nodesize = 5, nsplit = 10, importance = TRUE)
#tree = get.tree(obj, 1)
#tree

cox_fit = coxph(Surv(survival_time, dead_28) ~ total_LA_volume_sqrt_z + 
                  age_at_admission + gender + race + charlson_sqrt_z + apache3_sqrt_z + 
                  total_Kcal_sqrt_z, 
                data = coxdata)
...
## Cox-Snell residuals
coxdata$resid_coxsnell <- -(coxdata$resid_mart - coxdata$dead_28)
## Fit model on Cox-Snell residuals (Approximately Expo(1) distributed under correct model)
fit_coxsnell <- coxph(formula = Surv(resid_coxsnell, dead_28) ~ 1,
                      data    = coxdata,
...
ggsave("./Xinlei/Result/Diagnosis/Primay_Analysis_28_Day_Mortality_Among_Sepsis_martingale_plot.png", width = 10, height = 5)
...
### C-index
cindex = survConcordance(Surv(coxdata$survival_time, coxdata$dead_28) ~ predict(cox_fit))
print(cindex) # 0.77

ggsave("./Xinlei/Result/Diagnosis/Primay_Analysis_28_Day_Mortality_Among_Sepsis_cox_snell_plot.png", width = 8, height = 8)
...
ggsave("./Xinlei/Result/Diagnosis/Primay_Analysis_28_Day_Mortality_Among_Sepsis_ph_plot.png", width = 15, height = 15)

## Testing influential observations
ggcoxdiagnostics(cox_fit, type = "deviance", sline.se = F,
                 linear.predictions = FALSE, ggtheme = theme_bw())

ggsave("./Xinlei/Result/Diagnosis/Primay_Analysis_28_Day_Mortality_Among_Sepsis_deviance_plot.png", width = 8, height = 8)


# Save the result
write.csv(cox_fit_df, "./Xinlei/Result/Primay_Analysis_28_Day_Mortality_Among_Sepsis_cox_fit_result.csv")



# Cox imortal time bias
coxdata = data %>%
  select(survival_time, dead_28, total_LA_volume_sqrt_z, 
         age_at_admission, gender, race, charlson_sqrt_z, apache3_sqrt_z, 
         diabetes, SEPTIC_SHOCK, SEPSIS, vaso_inotr,
         total_Kcal_sqrt_z)
coxdata = coxdata[complete.cases(coxdata), ]
...
names(cox_fit_df) = c("Variable", "Coefficient", "Hazard Ratio", "Std. Err.", "Lower 95% CI", "Upper 95% CI", "P-value", "Time")

cox_fit_LA_volume = cox_fit_df %>%
  filter(Variable == "total_LA_volume_sqrt_z")

cox_fit_LA_volume %>%
  ggplot(aes(x = `Time`, y = `Hazard Ratio`)) +
  geom_point(aes(size = `Hazard Ratio`)) +
  geom_errorbar(aes(ymin=`Lower 95% CI`, ymax=`Upper 95% CI`), width=.2,
                position=position_dodge(0.05)) +
  geom_hline(yintercept = 1, linetype = "dashed") + 
  theme_classic() +
  theme(legend.position = "bottom") +
  labs(title = "Landmark Analysis",
       subtitle = "Hazard ratio of 28-day mortality at each landmark time")


ggsave("./Xinlei/Result/Plot/Primary_Analysis_28-Day_Mortality_Among_Sepsis_Landmark_Plot.png", width = 10, height = 8)