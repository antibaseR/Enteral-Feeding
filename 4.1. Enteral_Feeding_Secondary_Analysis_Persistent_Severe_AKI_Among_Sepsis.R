...
logidata = data %>%  
  mutate(AKI_Category_final = ifelse(AKI_Category_subgroup == "Persistent_Severe_AKI", "Yes", "No")) %>%
  select(AKI_Category_final, total_LA_volume_sqrt_z, 
...
test_sum_list = roc(testLabel, pred_prob$Yes) # 0.88
...
# Save the result
write.csv(glm_fit_df, "./Xinlei/Result/Secondary_Analysis_Persistent_Severe_AKI_Among_Sepsis_glm_fit_result.csv")
